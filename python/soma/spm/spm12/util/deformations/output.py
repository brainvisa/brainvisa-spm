...
class SaveDeformation(Output):
    ...
    def setOutputDestinationToOutputDirectory(self, ouput_directory):
        self.output_destination.setOutputDestinationToOutputDirectory(
            ouput_directory)
    ...
    def getStringListForBatch(self):
        if self.output_destination.current_directory or self.output_destination.ouput_directory is not None:
            ...
            batch_list.extend(addBatchKeyWordInEachItem(
                "savedef", self.output_destination.getStringListForBatch()))
            return batch_list
        else:
            raise ValueError(
    ...
    def getStringListForBatch(self):
        if self.volume_list_to_apply is not None:
            ...
            batch_list.append("pull.fnames = {%s};" % convertPathListToSPMBatchString(self.volume_list_to_apply,
                                                                                      add_dimension=False))
            batch_list.extend(addBatchKeyWordInEachItem(
                "pull", self.output_destination.getStringListForBatch()))
    ...
    def getStringListForBatch(self):
        if self.surface_path_list is not None:
            ...
            batch_list.append("surf.surface = {%s};" % convertPathListToSPMBatchString(self.surface_path_list,
                                                                                       add_dimension=False))
            batch_list.extend(addBatchKeyWordInEachItem(
                "surf", self.output_destination.getStringListForBatch()))
            return batch_list
        else:
    ...
    def getStringListForBatch(self):
        if self.current_directory or self.ouput_directory is not None:
            ...
            if self.current_directory:
                ...
                batch_list.append(
                    "savedir.saveusr = {'%s'};" % self.ouput_directory)
            return batch_list
        else:
            raise ValueError(
                "ouput directory path is required if output destination is not current directory")
...
class OutputDestinationWithSources(object):
    ...
    def getStringListForBatch(self):
        if self.current_directory or self.ouput_directory is not None or self.sources:
            batch_list = []
            if self.current_directory:
                batch_list.append("savedir.savepwd = 1;")
            elif self.sources:
            ...
            raise ValueError(
                "ouput directory path is required if output destination is not current directory or sources")
